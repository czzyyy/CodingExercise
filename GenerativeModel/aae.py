...
#     z = tf.placeholder(tf.float32, [batch_size, h_size])
#     sample_imgs = build_decoder(z)
#     saver = tf.train.Saver()
#     with tf.Session() as sess:
#         sess.run(tf.global_variables_initializer())
#         saver.restore(sess, '/root/aae_mnist/aae_mnist.ckpt')
#         sample_z = gaussian(batch_size, h_size)
#         samples = sess.run(sample_imgs, feed_dict={z: sample_z})[:num]
    ...
    with tf.Session() as sess:
        ...
        for e in range(max_epoch):
            for batch_i in range(total_batch):
                ...
                # real_z
                #real_z = gaussian(batch_size, h_size)
