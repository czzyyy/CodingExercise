...
class DC_GAN(object):
    ...
    def load_img(self, folder):
        ...
        for image in image_files:
            ...
            if im.shape != (self.input_size[0] * self.input_size[1], self.input_size[2]):
                #print('resize img')
    ...
    def build_generator(self, noise, train=True, reuse=False):
        with tf.variable_scope('generator', reuse=reuse):
            ...
            z = self.full_connect(noise, self.size * 4 * s_h8 * s_w8, name='g_full')
            # reshape成图像的格式
            h0 = tf.reshape(z, [-1, s_h8, s_w8, self.size * 4])
            # 对数据进行归一化处理 加快收敛速度
            h0 = self.batch_normalizer(h0, train=train, name='g_bn0')
            h0 = tf.nn.relu(h0, name='g_l0')

            h1 = self.deconv2d(h0, output_size=[self.batch_size, s_h4, s_w4, self.size * 2], name='g_h1')
            h1 = self.batch_normalizer(h1, train=train, name='g_bn1')
            h1 = tf.nn.relu(h1, name='g_l1')

            h2 = self.deconv2d(h1, output_size=[self.batch_size, s_h2, s_w2, self.size * 1], name='g_h2')
            ...
            h3 = self.deconv2d(h2, output_size=[self.batch_size, ] + self.input_size, name='g_h3')
            x_generate = tf.nn.tanh(h3, name='g_l3')
    ...
    def build_discriminator(self, imgs, reuse=False):
        with tf.variable_scope('discriminator', reuse=reuse):
            ...
            h3 = tf.reshape(h2, [self.batch_size, s_h8 * s_w8 * self.size * 4])

            h3 = self.full_connect(h3, 1, name='d_full')
            y_data = tf.nn.sigmoid(h3, name='d_l3')
    ...
    def generate_samples(self, num):
        ...
        with tf.Session() as sess:
            ...
            saver.restore(sess, '/root/small_dcgan.ckpt')
            sample_noise = np.random.uniform(-1.0, 1.0, size=(self.batch_size, self.noise_size))
            # samples = sess.run(sample_imgs, feed_dict={noise_imgs: sample_noise}).reshape([self.batch_size,
            #                                                                                self.input_size[0]
            #                                                                                * self.input_size[1],
            #                                                                                self.input_size[2]])[:num]
            samples = sess.run(sample_imgs, feed_dict={noise_imgs: sample_noise})[:num]
        for i in range(len(samples)):
            scipy.misc.imsave('F:/tf_board/small_dc_gan/' + str(i) + 'generate.png', samples[i])
            # im = Image.new("RGB", (self.input_size[0], self.input_size[1]), (0, 255, 0))
            # show_imgs = (samples[i] + 1.0) * (255.0 / 2.0)
            # one = [tuple(x) for x in np.array(show_imgs).astype(np.int32)]
            # im.putdata(one)
            # im.save('F:/tf_board/small_dc_gan/' + str(i) + 'generate.png')
        print('generate done!')
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            # choose dir
            writer = tf.summary.FileWriter('F:/tf_board/small_dc_gan', sess.graph)
            sess.run(tf.global_variables_initializer())
            for e in range(self.training_epochs):
                ...
                for batch_i in range(self.chunk_size):
                    ...
                    if (self.chunk_size * e + batch_i) % self.display_step == 0:
                        ...
                        scipy.misc.imsave('F:/tf_board/small_dc_gan/' +
                                          str(self.chunk_size * e + batch_i) + '-' + str(0) + '.png', show_imgs[0])
                        scipy.misc.imsave('F:/tf_board/small_dc_gan/' +
                                          str(self.chunk_size * e + batch_i) + '-' + str(1) + '.png', show_imgs[1])
                        # show_imgs = check_imgs[:2].reshape([2, self.input_size[0]
                        #                                     * self.input_size[1], self.input_size[2]])
                        # show_imgs = (show_imgs + 1.0) * (255.0 / 2.0)
                        #
                        # im1 = Image.new("RGB", (self.input_size[0], self.input_size[1]), (0, 255, 0))
                        # one = [tuple(x) for x in np.array(show_imgs[0]).astype(np.int32)]
                        # im1.putdata(one)
                        # im1.save('F:/tf_board/small_dc_gan/' + str(self.chunk_size * e + batch_i) + str(0) + '.png')
                        #
                        # im2 = Image.new("RGB", (self.input_size[0], self.input_size[1]), (0, 255, 0))
                        # two = [tuple(x) for x in np.array(show_imgs[1]).astype(np.int32)]
                        # im2.putdata(two)
                        # im2.save('F:/tf_board/small_dc_gan/' + str(self.chunk_size * e + batch_i) + str(1) + '.png')

            print('train done')
            # save sess
            saver.save(sess, '/root/small_dcgan.ckpt')

if __name__ == '__main__':
    data_folder = 'F:/python_code/images/faces'
    dcgan = DC_GAN(learning_rate=0.0002, noise_size=100, input_size=[48, 48, 3], training_epochs=30,
                   batch_size=64, display_step=128, size=16)
